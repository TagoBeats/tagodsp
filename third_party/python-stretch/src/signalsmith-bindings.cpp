...
struct Stretch{
    ...
    public:
        ...
        // Set formant shift (requires signalsmith-stretch with formant support)
        void setFormantFactor(Sample multiplier, bool compensatePitch=false) {
            stretch_.setFormantFactor(multiplier, compensatePitch);
        }
        void setFormantSemitones(Sample semitones, bool compensatePitch=false) {
            stretch_.setFormantSemitones(semitones, compensatePitch);
        }
        void setFormantBase(Sample baseFreq=0) {
            stretch_.setFormantBase(baseFreq);
        }
...
NB_MODULE(Signalsmith, m) {
    ...
    nb::class_<Stretch<Sample>>(m, "Stretch", "Class for Stretch processor.")
        ...
        .def("setTransposeSemitones", &Stretch<Sample>::setTransposeSemitones,
            ...
            "- tonalityLimit (float, optional): Restriction on tonal adjustments (default: 0).")
        .def("setFormantFactor", &Stretch<Sample>::setFormantFactor,
            "multiplier"_a, "compensatePitch"_a=false,
            "Set the formant shift as a frequency multiplier.\n\n"
            "Parameters:\n"
            "----------\n"
            "- multiplier (float): Formant shift multiplier (e.g., 2.0 shifts formants an octave up).\n"
            "- compensatePitch (bool, optional): If True, compensates formants for the pitch shift (default: False).")
        .def("setFormantSemitones", &Stretch<Sample>::setFormantSemitones,
            "semitones"_a, "compensatePitch"_a=false,
            "Set the formant shift in semitones.\n\n"
            "Parameters:\n"
            "----------\n"
            "- semitones (float): Number of semitones to shift the formants.\n"
            "- compensatePitch (bool, optional): If True, compensates formants for the pitch shift (default: False).")
        .def("setFormantBase", &Stretch<Sample>::setFormantBase,
            "baseFreq"_a=0,
            "Set the rough fundamental frequency used for formant analysis. 0 enables automatic pitch detection.")
